...
// 运行数据放 userData(%APPDATA%),打包后 app.asar 内不可写
...
function getBundledMihomoDir() {
  ...
  try {
    if (p && fs.existsSync(path.join(p, 'mihomo.exe'))) return p;
  } catch {}
  return path.join(__dirname, 'resources', 'mihomo');
}

// 定位内置 mihomo.exe:优先打包后的 resources/mihomo,回退开发目录
function resolveMihomoExe() {
  return path.join(getBundledMihomoDir(), 'mihomo.exe');
}
...
module.exports = { load, save, genApiKey, fetchSubscription, buildMihomoConfig, refreshMihomoConfig, CONFIG_FILE, MIHOMO_CONFIG, MIHOMO_DATA_DIR, LAST_NODE_FILE, USAGE_FILE, resolveMihomoExe, getBundledMihomoDir };
