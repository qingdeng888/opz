...
const PID_FILE = path.join(DATA_DIR, 'mihomo-zen.pid');
...
function start(exePath, logger) {
  return new Promise(async (resolve, reject) => {
    ...
    if (!fs.existsSync(exePath)) {
      return reject(new Error('找不到 mihomo 可执行文件: ' + exePath));
    }
    ...
    // 记录 PID,供 stop() 精确终止(跨平台)
    try { fs.writeFileSync(PID_FILE, String(child.pid), 'utf8'); } catch {}
    child.unref();
  ...
  return new Promise((resolve) => {
    try {
      if (fs.existsSync(PID_FILE)) {
        ...
        try { process.kill(pid, process.platform === 'win32' ? 'SIGKILL' : 'SIGTERM'); }
        catch (e) { logger('warn', '[mihomo] 进程可能已退出: ' + e.message); }
        try { fs.unlinkSync(PID_FILE); } catch {}
        logger('ok', `[mihomo] 已停止 (PID ${pid})`);
      } else {
        logger('warn', '[mihomo] 停止失败(未运行或无 PID 记录)');
      }
    } catch (e) {
      logger('warn', '[mihomo] 停止失败: ' + e.message);
    }
    resolve(true);
  });
}
